...
void mainImage( out vec4 fragColor, in vec2 fragCoord , vec3 rd )
{
...
//    vec2 mapUv = rd.xz;
//    vec2 uv = mapUv;
    vec2 mapUv =( fragCoord / iResolution.xy );// vec2(acos(rd.x) / PI, acos(rd.z) / PI);// * 2 - 1;
//    mapUv.y = -mapUv.y;

//    rd.xy
//    vec2 puv = normalize(rd.xy);
    vec2 tuv =vec2(acos(rd.x) / PI, acos(rd.y) / PI);//( fragCoord / iResolution.xy );
//    tuv.x = tuv.x * iResolution.x / iResolution.y;
    fragColor = vec4(tuv, 0, 1);
    return;
	vec2 position =mapUv;//rd.xy + 0.5;//mapUv;//rd.xy;//vec2((raydir.x + 2) * 0.5, raydir.y);// ( fragCoord / iResolution.xy );// vec2(raydir.x * 0.5 + 0.5, -raydir.y * 0.5 + 0.5);//vec2(1-acos(raydir.x) / 3.14, 1-asin(raydir.y)/ 3.14);// ( fragCoord / iResolution.xy );
