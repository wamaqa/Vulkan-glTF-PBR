...
{
   vec3 los = vec3(dir.x, dir.y, dir.z) * 2.0;
   float skyRange = max( dot( dir, vec3(0,1,0)), 0.0 );

   float coord1 =borealCloud(los * vec3(1.0* 0.4,0,1.0) + vec3(0.0,time *0.22,time *0.22));
   float coord2 =borealCloud(los * vec3(1.0* 0.7,0,0.7) + vec3(0.0,time *0.27,time *0.27));
   float coord3 =borealCloud(los * vec3(0.8* 0.6,0,0.6) + vec3(0.0,time *0.29,time *0.29));
   float coord4 =borealCloud(los * vec3(0.9* 0.5,0,0.5) + vec3(0.0,time *0.20,time *0.20));
  
   coord1  = min(pow(skyRange, 0.9), coord1);
   coord2  = min(pow(skyRange, 0.9), coord2);
   coord3  = min(pow(skyRange, 0.9), coord3);
   coord4  = min(pow(skyRange, 0.9), coord4);

   vec3 boreal =  vec3( .1, 1.,.5 ) * coord1 * 0.5 
                + vec3( .1,.9 ,.7) * coord2 * 0.9 
                + vec3(.75,.3 ,.99) * coord3 * 0.5 
                + vec3( .0,.99,.99) * coord4 * 0.57;


   SMOOTH_V3(los,.5,boreal);
   SMOOTH_V3(los,.35,boreal);
   SMOOTH_V3(los,.27,boreal);
...
//    float ex2 = 1.0;
//    float p = rayDir.y;
//    float p1 = 1.0f - pow (min (1.0f, 1.0f - p), ex1);
//    float p3 = 1.0f - pow (min (1.0f, 1.0f + p), ex1);
//    float p2 = 1.0f - p1 - p3;
//    float intensity = 0.1;
//    vec3 color1 = vec3(1.0, 1.0, 1.0);
//
//    vec3 skyCol = vec3(p1, p2, p3); //(color1 * p1 + color1 * p2 + color1 * p3) * intensity;
//    return skyCol;
...
//	sunColour = vec3(cloudParams.sunColourX, cloudParams.sunColourY, cloudParams.sunColourZ);
    ...
    vec3 boreal = renderBoreal(rayDir,iTime);
    vec3 sky = renderSky(rayDir,fract(iTime));
    vec3 moon =  renderMoon(sky, rayDir,normalize(vec3(1,0.5,1)));
    vec3 star =  renderStar(rayDir,fract(iTime));
    float ib = clamp(1.0-cIntensity(boreal)*3.,0.,1.);
    ib*=ib;
    float im = 1.0-cIntensity(moon);

    sky += ((moon)*ib*ib) +star +moon ;//+ boreal;

	outColor = vec4(sky, 1.0);//fragColor;
}
